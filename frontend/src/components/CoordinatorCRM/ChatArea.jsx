...
import { FaPaperPlane, FaPaperclip, FaSun, FaMoon, FaMicrophone, FaReply, FaTimes, FaImage, FaFilePdf, FaBolt, FaMagic, FaVideo, FaMusic, FaSmile } from 'react-icons/fa';
import toast from 'react-hot-toast';

const BACKEND_URL = 'http://72.62.249.211:5000'; 
...
export default function ChatArea({ selectedLead }) {
  ...
  const handleSend = async (e, forcedText = null, forcedFileBase64 = null) => {
    ...
    try {
      ...
      const res = await axios.post('/coordinator-crm/messages', formData, { headers: { 'Content-Type': 'multipart/form-data' } });
      
      if (selectedLead.id === targetLeadId) {
          setMessages([...messages, res.data]);
      }
      
      setNewMessage(''); setReplyingTo(null); setSelectedFile(null); setSuggestedQRs([]);
  ...
  const saveQuickReply = (e) => {
      ...
      setNewQRName(''); setNewQRMsg(''); setQrAttachmentBase64(null); toast.success("Saved!");
  };
      ...
      <div className={`flex flex-col border-t z-20 ${isDark ? 'bg-[#1e293b] border-slate-700' : 'bg-[#f0f2f5] border-slate-300'}`}>
              ...
              <div className="bg-[#0f172a] border-t border-slate-700 p-3 flex gap-2 overflow-x-auto custom-scrollbar shadow-lg">
                  {suggestedQRs.map((qr, i) => (
                      <button key={i} onClick={() => applyQuickReply(qr)} className="shrink-0 bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-xl text-xs font-bold transition-colors">
                          /{qr.name} {qr.attachment && <FaImage className="inline ml-1"/>}
                      </button>
                  ))}
              </div>
          )}
          ...
          <div className="p-3.5 flex items-end gap-3">
            ...
            {!newMessage.trim() && !selectedFile ? (
                <button type="button" className="p-4 bg-emerald-600/20 text-emerald-500 rounded-full hover:bg-emerald-600 hover:text-white transition-colors shadow-md"><FaMicrophone size={18}/></button>
            ) : (
         ...
         <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
             <div className="bg-[#1e293b] border border-slate-700 rounded-3xl p-6 w-full max-w-lg shadow-2xl">
                 ...
                 <form onSubmit={saveQuickReply} className="bg-[#0f172a] p-4 rounded-2xl border border-slate-700 mb-6">
                    ...
                    <div className="mb-4">
                        <label className="block text-xs font-bold text-slate-400 mb-2">Attachment (Optional - Max 2MB Image)</label>
                        <input type="file" accept="image/*" onChange={handleQRFile} className="w-full text-slate-400 text-xs file:mr-4 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:bg-blue-900 file:text-blue-400" />
                        {qrAttachmentBase64 && <div className="mt-2 text-xs text-emerald-400 flex items-center gap-1"><FaImage/> Image Attached</div>}
                    </div>
                     ...
                     {quickReplies.map((qr, i) => (
                         <div key={i} className="bg-[#0f172a] p-4 rounded-2xl border border-slate-700 flex justify-between items-start gap-4">
                             <div className="flex-1 min-w-0">
                                ...
                                {qr.attachment && <div className="text-emerald-400 text-[10px] font-bold mb-1 flex items-center gap-1"><FaImage/> Has Attachment</div>}
