...
export default function AfterSeminarNewInq({ leads, allBatches, filters, drafts, setDrafts, handleUpdateLocalLead, handleSaveCallData, handleTempUnlock, setChatModalLead, isManager, externalSearch }) {
  ...
  useEffect(() => { 
    ...
    const exportToExcel = () => {
        ...
        link.setAttribute("download", `Direct_Inquiries_Leads_Batch_${filters?.selectedBatch || 'All'}.csv`);
    ...
    const roundStats = useMemo(() => {
        const leadsInActiveRound = newInqLeads.filter(l => {
        ...
        if (name && name.trim()) {
            ...
            if (moveLeads) {
                ...
                try {
                    ...
                    await axios.post('/after-seminar-crm/leads/move-round', { targetRound: nextRoundNum, inquiryType: 'NEW_INQ' }, { headers: { Authorization: `Bearer ${token}` }});
    ...
    return (
        <div className="flex flex-col h-full space-y-4 animate-fade-in relative">
            ...
            <div className="flex justify-between items-center bg-[#141a23] p-4 rounded-2xl border border-slate-800 shadow-md">
                <div>
                    <p className="text-[10px] text-slate-400 font-semibold uppercase tracking-widest">Direct Inquiries Workspace</p>
                                ...
                                <React.Fragment key={lead.id}>
                                    ...
                                    `}>
                                        ...
                                        <div className="flex items-center gap-2.5 shrink-0">
                                            ...
                                            <button disabled={invalidPending} onClick={() => handleSaveNewInq(lead)} title={invalidPending ? "Change status before saving!" : "Save"} className={`p-3 rounded-lg transition-colors shadow-md ${invalidPending ? 'bg-slate-700 text-slate-500 cursor-not-allowed' : 'bg-emerald-600 hover:bg-emerald-500 text-white transform hover:scale-105'}`}><FaSave size={15}/></button>
